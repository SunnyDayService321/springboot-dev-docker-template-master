...
* **VPC Flow Logs**: ネットワークトラフィックのキャプチャによるセキュリティ監査・監視 (New)
...
### 5. セキュリティ & ネットワーク監視 (VPC Flow Logs)
システムのセキュリティ向上とネットワークトラブルシューティングのため、AWS実機環境にて **VPC Flow Logs（VPC フローログ）** を有効化しています。
* **導入の目的**:
  - **セキュリティ監査**: インターネットからVPC内に侵入する全トラフィック（ACCEPT / REJECT）を常時記録し、不正なスキャンやアタックを検知するため。
  - **トラブルシューティングの迅速化**: GitHub Actionsから本番EC2環境へSSH接続・SCP転送を行う際、通信障害（Time out等）が発生しても「パケットがセキュリティグループやACLのどのレイヤーで弾かれたか」をログの `action (ACCEPT/REJECT)` から即座に特定できる体制を整えました。
* **実機運用のこだわり**:
  - ログフォーマットには標準のメタデータ（送信元/送信先IP、ポート、プロトコル、アクション）を含め、ネットワークの疎通確認を完全にコントロールできる状態を証明しています。
* **主な活用例（検証実績）**:
- 外部IPからの特定ポートへの接続試行ログ（REJECT）の検知による、セキュリティグループ設定の正当性確認。
- CI/CDパイプライン実行時における、外部サーバーからEC2インスタンスへの通信状況の疎通確認。
